...
* 알림 테이블 생성
* 알림 함수 삽입 조회 함수 작성
* 구분 값 상관없이 현재 식재료 조회할 수 있는 함수 작성
* 테스트 데이터 저장
* AI & Bigdata와 연동부분 합치고 테스트
...
* ai부분 django rest framework 새로운 프로젝트로 만들어 분리하여 배포
* 센서/팔로우/사용자 부분 테스트 데이터 넣기
* 식재료 직접 입력하고 나서 빅데이터 함수 호출
...
* 냉장고 하드웨어 유지보수
* mqtt통신으로 냉장고 문 앱에서 제어
* 안드로이드 팔로우기능 통신 및 데이터 할당
* 불꽃 센서 값 들어올 시 안드로이드에서 알림
* 안드로이드 추천레시피 즐겨찾기 통신 및 
...
* 이미지 합성으로 데이터셋 생성
* 레이블링
* AI EC2로 AI application 옮겨서 Test
* 배포진행
...
* 사용자 정보 조회 함수 수정
* 추천레시피 조회 함수 생성
* 추천레시피 랜덤으로 1개 조회 함수 생성
* AI 분석을 통한 재료 데이터 저장 함수 테스트 완료
* 빅데이터 함수 호출하여 재료 추천레시피 저장 함수 테스트 완료
* 추천레시피 & 즐겨찾기 레시피 리턴 값 동일하게 나오도록 
...
* 라즈베리 파이 전구센서 연결
* 센서값 post요청 오류 해결
* 재료 위치 값을 통해 음식 아이콘 동적 할당



**박근웅**

* 라즈베리파이 or 만들어진 phsical 냉장고에서 사진 찍어 dataset 생성하기
* Labeling 하기
* Training 다시 시작하기
...
* 1인가구와 독서노인 데이터 수집 및 분석 (EDA)



**김지현**

* 안드로이드 식재료 값 조회 통신 및 데이터할당
* 리허설 발표 시연영상 촬영
